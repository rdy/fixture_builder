...
Every generated YAML file starts with
`# Generated by fixture_builder. Do not edit.`. FixtureBuilder uses that exact first
line to identify its output. During a rebuild it removes marked files that are no
longer produced, including fixtures for dropped or newly skipped tables. The explicit
`spec:fixture_builder:clean` task likewise removes only marked YAML files. Unmarked
files are preserved when stale.

When a current output path contains an unmarked file, FixtureBuilder warns before
replacing it and continues the build. A marked file remains generated output even if
its YAML was edited, so a later build may overwrite or delete it. Existing version 1
manifests without marker adoption rebuild once and migrate quietly. Missing, invalid,
or already-adopted manifests do not suppress collision warnings.

The default output directory remains the Rails fixture directory. To keep generated
and hand-authored fixtures separate, configure a sibling directory and register both
fixture roots with your test framework. Do not generate the same fixture-set name in
both roots. For example, if `countries.yml` is hand-authored, skip that table so an
empty generated file does not duplicate it:

```ruby
FixtureBuilder.configure do |fbuilder|
  fbuilder.fixture_directory = Rails.root.join("test/generated_fixtures")
  fbuilder.skip_tables += ["countries"]
end
```

For Minitest, register the generated root in `test/test_helper.rb` after
`rails/test_help` and before declaring `fixtures :all`:

```ruby
require "rails/test_help"

class ActiveSupport::TestCase
  self.fixture_paths += [Rails.root.join("test/generated_fixtures")]
  fixtures :all
end
```

For RSpec with rspec-rails 6.1 or newer, configure both roots in
`spec/rails_helper.rb`:
...
RSpec.configure do |config|
  config.fixture_paths = [
    ...
    Rails.root.join("spec/generated_fixtures")
  ]
  config.global_fixtures = :all # Optional existing RSpec behavior
end
```

Test-loader registration does not configure Rails database tasks. `FIXTURES_PATH` is
one root-relative directory, not a list. To load only generated fixtures in
development, run:
...
* To set up associations between different types of created fixture model objects, you can
  ...
  1. Use `MyModel.find_by!(some_unique_field: value)` to find a previously created instance that
     didn't have a name.
* When a table is deleted or skipped, the next rebuild removes its old marked fixture.
  Unmarked stale files are preserved for manual inspection.
